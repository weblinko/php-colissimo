...
/**
 * This class stands for MTOM ResponseReader
 */
class MTOM_ResponseReader
{
   ...
   private function soapToObject($xml)
   {
      ...
      return (object)$this->arrayToObjectRecursiveData($xml_to_array);
   }
    ...
    * @return mixed|object
    */
   private function arrayToObjectRecursiveData($arr)
   {
      if (is_array($arr)) {
         $generate_array = array();
         foreach ($arr as $k => $v) {
            if (is_integer($k)) {
               $generate_array['index'][$k] = $this->arrayToObjectRecursiveData($v);
            } else {
               $generate_array[$k] = $this->arrayToObjectRecursiveData($v);
            }
         }
